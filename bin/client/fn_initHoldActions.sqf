...
if ((player getVariable "gameSide") == "defenders") then {
	...
	_cond = "(player distance2D sv_cur_obj) < ceil(([sv_cur_obj] call client_fnc_getObjectiveDistance) + 1) && {(sv_cur_obj getVariable ['status',-1] == 1) || (sv_cur_obj getVariable ['status', -1] == 0 && (_this isEqualTo player))}";
	_completion = {if ((sv_cur_obj distance2D player) < ceil(([sv_cur_obj] call client_fnc_getObjectiveDistance) + 1)) then {[] call client_fnc_disarmMCOM;};};
	_interruption = {sv_cur_obj setVariable ["status", 1, true]};
} else {
	_icon = _icon + "bombPlant.paa";
	_text = "Plant Explosives";
	_cond = "(player distance2D sv_cur_obj) < ceil(([sv_cur_obj] call client_fnc_getObjectiveDistance) + 1) && {(sv_cur_obj getVariable ['status',-1] == -1) || (sv_cur_obj getVariable ['status', -1] == 2) || ((sv_cur_obj getVariable ['status', -1] == 0) && (_this isEqualTo player))}";
	_completion = {if ((sv_cur_obj distance2D player) < ceil(([sv_cur_obj] call client_fnc_getObjectiveDistance) + 1)) then {[] call client_fnc_armMCOM;};};
	_interruption = {sv_cur_obj setVariable ["status", -1, true]};
};
